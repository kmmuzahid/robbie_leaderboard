...
class _PhoneNumberFieldWidgetState extends State<PhoneNumberFieldWidget> {
  ...
  void initState() {
    ...
    _initalPhone = PhoneNumber(
        phoneNumber: widget.initialValue?.phoneNumber,
        isoCode: widget.initialValue?.isoCode,
        dialCode: widget.initialValue?.dialCode);
    if (widget.initialValue?.phoneNumber?.isNotEmpty == true) {
      String phone =
          widget.initialValue!.phoneNumber!.replaceFirst(widget.initialValue!.dialCode!, '');
      phone = phone.replaceFirst('+', '');
      widget.controller.text = phone;
      // PhoneNumber.getRegionInfoFromPhoneNumber('+35811111111111111').then(
      //   (value) {
      //     print(value.isoCode);
      //   },
      // );
    ...
    print(_initalPhone.dialCode);
    print(_initalPhone.phoneNumber);
    // print(_initalPhone.isoCode);
  }
          ...
          child: InternationalPhoneNumberInput(
            onInputChanged: (PhoneNumber number) {
              setState(() {
                ...
                if (_initalPhone.isoCode != number.isoCode) {
                  _initalPhone = PhoneNumber(
